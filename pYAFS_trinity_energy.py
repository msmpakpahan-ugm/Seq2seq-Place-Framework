...
def KPI_processv3(folder_file, folderapps, folderpops, place_folder_name, nodes_folder_name, pop_ids, app_ids, method_name, number_of_modules=None):
  ...
  average_comm_energy_list = []
  average_proc_energy_list = []
  average_total_energy_list = []
  ...
  for id_app in (app_list):
    ...
    # Append module count once per app
    modules.append(current_number_of_module)
    ...
    for id_process in id_app_list:
        ...
        if len(framed_process) >= current_number_of_module:
            ...
            # Calculate transmission time based on whether it's a cloud link
    ...
    if(len(coms) != 0):
        ...
        average_response_time.append(sum(response_total)/num_valid_coms)
        average_adra.append(sum(total_adra)/num_valid_coms)
        avg_adsa_val = sum(total_adsa)/num_valid_coms if total_adsa else 0
        ...
        if len(total_hop) == num_valid_coms:
            ...
            average_wait.append(max(0, wait))
            
            # Add average energy metrics to lists
