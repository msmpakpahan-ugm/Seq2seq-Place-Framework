...
class ExternalContext:
   def __init__(self, app_df, nodes_df, cloud_node_id, keep_duplicate_failures=False):
       self.app_df = app_df
       ...
       self.max_retries = 5
       self.keep_duplicate_failures = keep_duplicate_failures  # New option
       self.prepare_data()
   ...
   def record_node_failure(self, app_id, module_id, node_id, reason="Unknown"):
        """Record a node failure with reason for the current placement attempt.
           If keep_duplicate_failures is False, ensures summary node failure entries (f<node_id>) are unique,
        ...
        if self.keep_duplicate_failures or failure_entry_summary not in self.current_failures[app_id]:
...
def place_seq2seq_externalcontext_v2(csv_file_path, encoder, decoder, encoder_voc, decoder_voc,
                                     ...
                                     keep_duplicate_failures=False, # New parameter to control failure handling
                                     output_path=None):
    """
    ...
    Args:
        csv_file_path: Path to the CSV file containing application data
        encoder: The encoder model
        decoder: The decoder model
        encoder_voc: Encoder vocabulary
        decoder_voc: Decoder vocabulary
        external_context: ExternalContext instance for managing resources and failures
        original_decode_with_context_check_fn: Original decode function for comparison
        MAX_MODEL_ATTEMPTS: Maximum number of model-driven attempts before cloud fallback
        keep_duplicate_failures: If True, allows duplicate failure entries in the failure string
        output_path: Path to save the results (optional)
    """
    # Set the keep_duplicate_failures option in external_context
    external_context.keep_duplicate_failures = keep_duplicate_failures
    
    df = pd.read_csv(csv_file_path)
    results = []
    # MAX_MODEL_ATTEMPTS = 6 # Total model-driven attempts before cloud fallback # This is now a parameter
    ...
    for index, row in df.iterrows(): # Main loop for each application
        ...
        for attempt_num in range(1, MAX_MODEL_ATTEMPTS + 1):
            ...
            # The logic for switching decode functions might need to be adjusted if MAX_MODEL_ATTEMPTS changes significantly.
            # For example, if MAX_MODEL_ATTEMPTS is less than the threshold for switching.
            # Current logic: use early stop for N-1 attempts, then original for the last model attempt.
            # If MAX_MODEL_ATTEMPTS is 1, it will use original_decode_with_context_check_fn.
            # If MAX_MODEL_ATTEMPTS > 1, it will use decode_with_early_failure_report_check for attempts 1 to MAX_MODEL_ATTEMPTS-1,
            # and original_decode_with_context_check_fn for attempt MAX_MODEL_ATTEMPTS.
            if attempt_num < MAX_MODEL_ATTEMPTS : # Use early stop for all but the last model attempt
                decode_fn_to_use = decode_with_early_failure_report_check
                decode_fn_name_for_log = "EarlyStopDecode"
            else: # Last model attempt (attempt_num == MAX_MODEL_ATTEMPTS)
