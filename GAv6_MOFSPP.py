...
class GeneticAlgorithmv5additionalfitness:
    ...
    def run(self, num_iterations):
        ...
        for iteration in range(num_iterations):
            ...
            valid_individuals = []
            valid_objective_values = {'response_time': [], 'cost': [], 'energy': []}
            all_violations = []

            for chromo, components in population_components:
                violation = components['storage_violation_amount']
                all_violations.append(violation)
                # Check if violation is effectively zero (handle potential float issues)
                if violation < 1e-9:
                    valid_individuals.append((chromo, components))
                    valid_objective_values['response_time'].append(components['total_response_time'])
                    valid_objective_values['cost'].append(components['total_cost'])
                    valid_objective_values['energy'].append(components['total_energy'])

            # 3. Determine normalization ranges based ONLY on valid individuals
            min_max = {}
            for obj in ['response_time', 'cost', 'energy']:
                values = valid_objective_values[obj]
                min_max[obj] = {'min': min(values) if values else 0,
                                'max': max(values) if values else 0}

            rt_range = min_max['response_time']['max'] - min_max['response_time']['min']
            cost_range = min_max['cost']['max'] - min_max['cost']['min']
            energy_range = min_max['energy']['max'] - min_max['energy']['min']

            # Avoid division by zero if all valid individuals have the same value for an objective
            ...
            # # 4. Find max violation across ALL individuals for penalty normalization
            # max_storage_violation = max(all_violations) if all_violations else 0
            # # Avoid division by zero if max violation is 0
            # max_storage_violation_norm_divisor = max_storage_violation if max_storage_violation > 1e-9 else 1.0

            # 5. Calculate final fitness for ALL individuals
            population_fitness_results = []
            for chromo, components in population_components:
                # Normalize objectives using ranges from VALID individuals
                norm_rt = (components['total_response_time'] - min_max['response_time']['min']) / rt_range
                norm_cost = (components['total_cost'] - min_max['cost']['min']) / cost_range
                norm_energy = (components['total_energy'] - min_max['energy']['min']) / energy_range

                # --- Calculate fitness based on normalized objectives --- 
                obj_denominator = norm_rt + norm_cost + norm_energy
                if obj_denominator < 1e-9:
                    # Handle case where all normalized objectives are zero (perfect on objectives)
                    # Assign a very large base fitness, penalty will be subtracted later
                ...
                # Ensure fitness is not infinite if base_fitness_part was infinite but there's a penalty
                if violation > 1e-9 and base_fitness_part == float('inf'):
                    # If objectives were perfect but there's a violation,
                    # we need a defined large number instead of inf - penalty.
                    # We could use a large number like LARGE_PENALTY_VALUE negated, 
                    # or just subtract the weighted penalty from a predefined large positive number.
                    # Let's subtract from a large number to keep it relative. 
                    # Using 1 / (W_STORAGE_PENALTY * violation) might make sense, 
                    # but let's use a simpler large number approach first.
                    # Redefine fitness if it resulted from inf - penalty. Use a large value minus penalty.
                     final_fitness = (1.0 / 1e-9) - W_STORAGE_PENALTY * violation # Use a large defined value
                
                # Store results: chromosome, (final_fitness, original_components)
            ...
            if current_best_fitness > best_fitness_overall:
                ...
                # Store the components dictionary associated with the best fitness
            ...
            # --- Print Progress ---
            # Print final fitness score and raw components of the generation's best
            print(f'Iteration {iteration}: Best Fitness = {current_best_fitness:.4f}, Overall Best Fitness = {best_fitness_overall:.4f}')
            print("  Current Best Components (Raw):")
            temp_best_components = sorted_population_tuples[0][1][1] # Get components dict
            for key, value in temp_best_components.items():
                if key == 'final_fitness_score': continue
                if key == 'storage_violation_amount':
                    print(f"    Storage Violation: {value:.4f}")
        ...
        if best_chromosome_overall and best_components_overall:
            # Update the description of the fitness value printed
            print(f"  Overall Best Fitness (Calculated as [3 / norm_objs] - W_penalty * raw_violation): {best_fitness_overall:.4f}")
            print("  Best Solution Components (Raw):")
            for key, value in best_components_overall.items():
                if key == 'final_fitness_score': continue
                if key == 'storage_violation_amount':
                    print(f"    Storage Violation: {value:.4f}")
                elif isinstance(value, (int, float)):
