...
def KPI_processv3(folder_file, folderapps, folderpops, place_folder_name, nodes_folder_name, pop_ids, app_ids, method_name, number_of_modules=None):
  ...
  cloud_bw = 125000  # Cloud bandwidth (bytes per second)
  edge_bw = 75000    # Edge bandwidth (bytes per second)
  edge_tx_factor = 5.5  # Edge transmission power factor (watts)
  edge_rx_factor = 4.5  # Edge reception power factor (watts)
  cloud_tx_factor = 40  # Cloud transmission power factor (watts)
  cloud_rx_factor = 30  # Cloud reception power factor (watts)
  cloud_processing_factor = 250  # Cloud processing power factor (watts per instruction)
  edge_processing_factor = 65    # Edge processing power factor (watts per instruction)
  ...
  # Energy metrics lists
  total_comm_energy = []  # List to store communication energy for each process
  total_proc_energy = []  # List to store processing energy for each process
  total_energy = []      # List to store total energy for each process

  # Initialize energy lists at the app level
  average_comm_energy_list = []  # List to store average communication energy per app
  average_proc_energy_list = []  # List to store average processing energy per app
  average_total_energy_list = [] # List to store average total energy per app
  ...
  for id_app in (app_list):
    ...
    for id_process in id_app_list:
      ...
      # print(f"number of module: {number_of_module}")
        if len(framed_process) >= current_number_of_module:
            ...
            # Step 1: Communication Energy Calculation
            # 1.1 Calculate total bytes transmitted in this process
            process_bytes = sum(framed_link_id['size'])
            
            # 1.2 Determine if any links involve cloud node
            is_cloud_link = (framed_link_id['src'] == cloud_node_id) | (framed_link_id['dst'] == cloud_node_id)
            
            # 1.3 Calculate transmission time based on bandwidth
            # If any link involves cloud, use cloud bandwidth, otherwise use edge bandwidth
            transmission_time = process_bytes / cloud_bw if is_cloud_link.any() else process_bytes / edge_bw
            
            # 1.4 Determine if source or destination is cloud
            is_source_cloud = (framed_link_id['src'] == cloud_node_id).any()
            is_dest_cloud = (framed_link_id['dst'] == cloud_node_id).any()
            
            # 1.5 Calculate transmission and reception power
            # Use cloud factors if source/destination is cloud, otherwise use edge factors
            tx_power = transmission_time * (cloud_tx_factor if is_source_cloud else edge_tx_factor)
            rx_power = transmission_time * (cloud_rx_factor if is_dest_cloud else edge_rx_factor)
            
            # 1.6 Calculate total communication energy for this process
            process_comm_energy = tx_power + rx_power
            total_comm_energy.append(process_comm_energy)
            
            # Step 2: Processing Energy Calculation
            # 2.1 Calculate service time for cloud and edge nodes separately
            ...
            # 2.2 Calculate processing energy using appropriate factors
            # Cloud processing energy = cloud service time * cloud processing factor
            # Edge processing energy = edge service time * edge processing factor
            ...
            # 3.1 Calculate total energy for this process
    ...
    if(len(coms) != 0):
        ...
        if len(total_hop) == num_valid_coms:
          ...
          # Step 4: Calculate Average Energy per Application
          # 4.1 Calculate average communication energy
          average_comm_energy_list.append(sum(total_comm_energy)/num_valid_coms)
          # 4.2 Calculate average processing energy
          average_proc_energy_list.append(sum(total_proc_energy)/num_valid_coms)
          # 4.3 Calculate average total energy
          average_total_energy_list.append(sum(total_energy)/num_valid_coms)
          ...
          max_buffer.append(0)
          average_comm_energy_list.append(0)
          average_proc_energy_list.append(0)
          average_total_energy_list.append(0)
  ...
  trans = pd.DataFrame(list(zip(app_list, modules, number_coms, average_response_time,
                              ...
                              max_buffer, lambda_values, average_comm_energy_list,
                              average_proc_energy_list, average_total_energy_list)),
