...
def evaluate(encoder, decoder, encoder_voc, decoder_voc, sentence, max_length=MAX_LENGTH, temperature=1.0, top_k=5, beam_size=1):
    # Tokenize the input sentence based on the tokenizer type
    ...
    if beam_size == 1:
        decoded_words, _ = decode(decoder, decoder_hidden, encoder_outputs, decoder_voc, max_length, temperature, top_k)
        output_sentence = ' '.join(decoded_words)
        return output_sentence
    else:
        beam_results = beam_decode(decoder, decoder_hidden, encoder_outputs, decoder_voc, beam_size, max_length)
        # Return the top result from beam search
        top_result = beam_results[0]
        output_sentence = ' '.join(top_result[0])
        return output_sentence
...
def trainIters(model_name, encoder_voc, decoder_voc, pairs, encoder, decoder, encoder_optimizer, decoder_optimizer, encoder_embedding, decoder_embedding, 
    ...
    for iteration in range(start_iteration, n_iteration + 1):
        ...
        loss = train(input_variable, lengths, target_variable, mask, max_target_len, encoder,
                     decoder, encoder_embedding, decoder_embedding, encoder_optimizer, decoder_optimizer, 
